...
def convert_with_libreoffice(
    ...
    if target.suffix.lower() == ".pdf" and expected_pages is not None:
        actual_pages = len(PdfReader(target).pages)
        if actual_pages != expected_pages:
            raise ConversionError(
                f"Generated PDF contains {actual_pages} pages instead of {expected_pages}."
            )
    return target
...
def render_pdf_pages(
    ...
    if expected_pages is not None and len(pages) != expected_pages:
        raise ConversionError(
            f"Expected {expected_pages} rendered slides; found {len(pages)}."
        )
    return pages
