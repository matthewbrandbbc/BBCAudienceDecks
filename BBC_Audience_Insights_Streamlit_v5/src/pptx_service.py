...
COMMON_TEXT_REPLACEMENTS = {
    "BBC’s All Audiences  Audience": "the Total BBC Audience",
    "BBC’s All Audiences Audience": "the Total BBC Audience",
    "BBC’s All Audiences": "the Total BBC Audience",
    "BBC All Audiences": "Total BBC Audience",
    "avg. All Audiences  Audience": "avg. Total Audience",
    "avg. All Audiences Audience": "avg. Total Audience",
    "avg. All Audiences": "avg. Total Audience",
    "All Audience Average": "Total Audience Average",
}
...
def _remove_suppressed_items(root, markers: set[str], slide_number: int):
    ...
    # Slide 4 holds two statistics in each text box. Remove the number, label
    ...
    # Slides 9 and 10 use self-contained grouped cards. Remove the
    # complete card so no orphan label remains after a statistic is suppressed.
    if slide_number in {9, 10}:
...
def _rank_employment_rows(root, row_groups: list[list[int]]):
    """Reorder complete Slide 6 table rows within each existing section."""
    for table in root.xpath(".//a:tbl", namespaces=NS):
        rows = list(table.findall(f"./{A}tr"))
        marker_rows: dict[int, etree._Element] = {}
        positions: dict[int, int] = {}
        for position, row_element in enumerate(rows):
            text = "".join(row_element.xpath(".//a:t/text()", namespaces=NS))
            match = re.search(r"\(I(9[8-9]|10[0-7])\)", text)
            if match:
                source_row = int(match.group(1))
                marker_rows[source_row] = row_element
                positions[source_row] = position
        replacements: dict[int, etree._Element] = {}
        for desired_rows in row_groups:
            available = [row for row in desired_rows if row in marker_rows]
            if not available:
                continue
            target_positions = sorted(positions[row] for row in available)
            for target_position, source_row in zip(target_positions, available):
                replacements[target_position] = deepcopy(marker_rows[source_row])
        for position, new_row in replacements.items():
            table.replace(rows[position], new_row)


def _retain_visible_slides(package: dict[str, bytes], visible_numbers: set[int]):
    """Remove hidden slides from the presentation order while retaining shared assets."""
    presentation = etree.fromstring(package["ppt/presentation.xml"])
    relationships = etree.fromstring(package["ppt/_rels/presentation.xml.rels"])
    rel_tag = f"{{{PACKAGE_REL_NS}}}Relationship"
    by_id = {rel.get("Id"): rel for rel in relationships.findall(rel_tag)}
    slide_list = presentation.find(f"./{P}sldIdLst")
    if slide_list is None:
        raise ExportValidationError("The template does not contain a slide list.")
    for slide_id in list(slide_list):
        rel_id = slide_id.get(f"{R}id")
        relationship = by_id.get(rel_id)
        target = relationship.get("Target", "") if relationship is not None else ""
        match = re.search(r"slides/slide(\d+)\.xml$", target)
        if match and int(match.group(1)) not in visible_numbers:
            slide_list.remove(slide_id)
            relationships.remove(relationship)
    package["ppt/presentation.xml"] = etree.tostring(
        presentation, xml_declaration=True, encoding="UTF-8", standalone=True
    )
    package["ppt/_rels/presentation.xml.rels"] = etree.tostring(
...
def _apply_footer(slide_root, footer_template_root, lines: list[str], slide_number: int):
    ...
    # Replace the sample footer in its original template position.
    for shape in list(sp_tree):
        name = shape.xpath("./p:nvSpPr/p:cNvPr/@name", namespaces=NS) if hasattr(shape, "xpath") else []
        text = "".join(shape.xpath(".//a:t/text()", namespaces=NS)) if hasattr(shape, "xpath") else ""
        if (name and name[0].startswith("Generated Footer")) or text.startswith("Base: All Audiences"):
...
def _brand_axis_maximum(items: list[dict]) -> float:
    """Return a stable, readable maximum while leaving headroom for labels."""
    highest = max((float(item["value"]) for item in items), default=100.0)
    return float(max(120, math.ceil((highest * 1.1) / 20) * 20))


def _set_value_axis_bounds(root, maximum: float):
    scaling = root.find(".//c:valAx/c:scaling", namespaces=NS)
    if scaling is None:
        return
    minimum_node = scaling.find(f"./{C}min")
    if minimum_node is None:
        minimum_node = etree.SubElement(scaling, f"{C}min")
    minimum_node.set("val", "0")
    maximum_node = scaling.find(f"./{C}max")
    if maximum_node is None:
        maximum_node = etree.SubElement(scaling, f"{C}max")
    maximum_node.set("val", f"{maximum:g}")


def _position_brand_average_marker(slide_root, axis_maximum: float):
    """Align the dotted guide and its label with index 100 on slide 8."""
    chart_frames = slide_root.xpath(".//p:graphicFrame[.//c:chart]", namespaces=NS)
    if not chart_frames:
        return
    chart_frame = chart_frames[0]
    chart_xfrm = chart_frame.find(f"./{P}xfrm")
    if chart_xfrm is None:
        return
    chart_off = chart_xfrm.find(f"./{A}off")
    chart_ext = chart_xfrm.find(f"./{A}ext")
    if chart_off is None or chart_ext is None:
        return

    # These fractions describe the existing chart's native plot area. The
    # value axis is made explicit above, so the guide can be placed from the
    # data value instead of remaining at a fixed slide coordinate.
    plot_top = 0.264
    plot_bottom = 0.803
    value_fraction = min(1.0, max(0.0, 100.0 / axis_maximum))
    y_fraction = plot_bottom - value_fraction * (plot_bottom - plot_top)
    marker_y = round(int(chart_off.get("y", "0")) + int(chart_ext.get("cy", "0")) * y_fraction)

    for connector in slide_root.xpath(".//p:cxnSp[.//a:prstDash]", namespaces=NS):
        off = connector.find(f"./{P}spPr/{A}xfrm/{A}off")
        if off is not None:
            off.set("y", str(marker_y))

    for shape in slide_root.xpath(".//p:sp", namespaces=NS):
        text = "".join(shape.xpath(".//a:t/text()", namespaces=NS)).strip()
        if text in {"All Audience Average", "Total Audience Average"}:
            off = shape.find(f"./{P}spPr/{A}xfrm/{A}off")
            if off is not None:
                off.set("y", str(marker_y))
...
def _update_chart_xml(
    ...
    for formula in root.findall(".//c:ser/c:cat/c:strRef/c:f", namespaces=NS):
        formula.text = re.sub(r"\$A\$2:\$A\$\d+", f"$A$2:$A${len(items) + 1}", formula.text or "")
    for formula in root.findall(".//c:ser/c:val/c:numRef/c:f", namespaces=NS):
        formula.text = re.sub(r"\$B\$2:\$B\$\d+", f"$B$2:$B${len(items) + 1}", formula.text or "")
    labels_parent = root.find(".//c:ser/c:dLbls", namespaces=NS)
    if compact_labels and labels_parent is not None:
        existing = labels_parent.findall(f"./{C}dLbl")
        prototype = deepcopy(existing[0]) if existing else None
        for label in existing:
            labels_parent.remove(label)
        if prototype is not None:
            for idx, item in enumerate(items):
                label = deepcopy(prototype)
                label.find(f"./{C}idx").set("val", str(idx))
                nodes = label.xpath(".//a:t", namespaces=NS)
                if nodes:
                    nodes[0].text = item["label"]
                    for node in nodes[1:]:
                        node.text = ""
                labels_parent.insert(idx, label)
    ...
    if axis_maximum is not None:
        _set_value_axis_bounds(root, axis_maximum)
    _set_fonts(root)
...
def generate_pptx(model: PresentationModel, output_path: Path | None = None) -> Path:
    ...
    slide_by_number = {slide.number: slide for slide in model.slides}
    _retain_visible_slides(package, set(slide_by_number))
    for slide in model.slides:
        name = f"ppt/slides/slide{slide.number}.xml"
        root = etree.fromstring(package[name])
        if slide.number == 6:
            _rank_employment_rows(root, slide.charts.get("row_groups", []))
        _remove_suppressed_items(root, slide.suppressed_markers, slide.number)
        _replace_text_nodes(root, slide.replacements)
        _replace_text_nodes(root, COMMON_TEXT_REPLACEMENTS)
        _remove_duplicate_suppression_messages(root)
        if slide.number == 3:
            _remove_empty_charts(root, slide.charts)
        if slide.number == 8 and not slide.charts["Brand Discovery"]["items"]:
            for frame in list(root.xpath(".//p:graphicFrame[.//c:chart]", namespaces=NS)):
                frame.getparent().remove(frame)
        if slide.footer:
            _apply_footer(root, root, slide.footer, slide.number)
    ...
    slide3 = slide_by_number[3]
    for platform, (chart_path, workbook_path) in chart_names.items():
        chart = slide3.charts[platform]
        package[chart_path] = _update_chart_xml(package[chart_path], chart, compact_labels=True)
        package[workbook_path] = _update_embedded_workbook(package[workbook_path], chart)

    brand_chart = slide_by_number[8].charts["Brand Discovery"]
    brand_axis_maximum = _brand_axis_maximum(brand_chart["items"])
    package["ppt/charts/chart4.xml"] = _update_chart_xml(
        package["ppt/charts/chart4.xml"], brand_chart, axis_maximum=brand_axis_maximum
    )
    slide8_root = etree.fromstring(package["ppt/slides/slide8.xml"])
    _position_brand_average_marker(slide8_root, brand_axis_maximum)
    package["ppt/slides/slide8.xml"] = etree.tostring(
        slide8_root, xml_declaration=True, encoding="UTF-8", standalone=True
    )
    package["ppt/embeddings/Microsoft_Excel_Worksheet3.xlsx"] = _update_embedded_workbook(
        package["ppt/embeddings/Microsoft_Excel_Worksheet3.xlsx"], brand_chart
    )
    ...
    validate_pptx(output_path, expected_slides=len(model.slides))
    return output_path


def validate_pptx(path: Path, expected_slides: int | None = None):
    marker = re.compile(r"\([A-W]{1,2}\d{1,3}\)")
    with ZipFile(path) as z:
        presentation = etree.fromstring(z.read("ppt/presentation.xml"))
        presentation_rels = etree.fromstring(z.read("ppt/_rels/presentation.xml.rels"))
        target_by_id = {
            rel.get("Id"): rel.get("Target", "")
            for rel in presentation_rels.findall(f"{{{PACKAGE_REL_NS}}}Relationship")
        }
        ...
        if expected_slides is not None and slide_count != expected_slides:
            raise ExportValidationError(
                f"Generated PPTX contains {slide_count} visible slides instead of {expected_slides}."
            )
        slides = []
        for slide_id in slide_ids:
            target = target_by_id.get(slide_id.get(f"{R}id"), "")
