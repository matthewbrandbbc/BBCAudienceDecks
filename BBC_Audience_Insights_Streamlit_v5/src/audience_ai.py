...
class GroundedAudienceEngine:
    ...
    def opportunity_score(self, region: str, audience: str, platform: str):
        ...
        if not (reach and competitive and affinity):
            return None
        rate = float(re.search(r"\((\d+)%", reach.value).group(1))
        rank = int(re.search(r"(\d+)", competitive.value).group(1))
        scale_points = min(40, rate / 50 * 40)
        competitive_points = max(0, (7 - rank) / 6 * 30)
        affinity_points = min(30, max(0, affinity.index - 100) / 100 * 30)
        score = round(scale_points + competitive_points + affinity_points)
        return score, round(scale_points), round(competitive_points), round(affinity_points), affinity
    ...
    def overview(self, region: str, audience: str, platform: str) -> AudienceOverview:
        ...
        score = self.opportunity_score(region, audience, platform)
        if score:
            bullets.append(f"Directional sales opportunity score: {score[0]}/100 (scale {score[1]}, competitive position {score[2]}, distinctiveness {score[3]}).")
        if not bullets:
        ...
        return AudienceOverview(
            headline=f"{audience} in {region} — grounded sales overview",
    ...
    def suggested_questions(self, region: str, audience: str, platform: str) -> tuple[str, ...]:
        return (
            f"What are the top indexing qualities for {audience} in {region}?",
            f"What is the strongest sales argument for {audience} on {platform}?",
            f"What is the directional opportunity score for {audience} in {region}?",
        )
    ...
    def answer(
        ...
        if any(term in normal for term in ("opportunity score", "sales score", "how strong an opportunity")):
            score = self.opportunity_score(region, audience, platform)
            if not score:
                return GroundedAnswer(f"A directional opportunity score is unavailable because one or more required response bases are below {MIN_RESPONSES}.", resolved_scope=scope)
            return GroundedAnswer(
                f"The directional sales opportunity score is **{score[0]}/100**: scale {score[1]}/40, competitive position {score[2]}/30 and distinctiveness {score[3]}/30. It is a transparent prioritisation aid, not a sales forecast.",
        ...
        return GroundedAnswer(
            "I can answer grounded questions about reach, competitive position, top affinities, "
            "indexes and the strongest sales argument. Try “What is the strongest argument for "
            "BBC audiences in Latin America?” or “Which audience has the strongest digital media "
            "affinity in Europe?”",
            resolved_scope=scope,
        )
