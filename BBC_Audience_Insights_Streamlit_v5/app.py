...
st.markdown(
    ...
    <style>
      ...
      .interpretation-note {background:#ececec;border-radius:4px;padding:12px 14px;margin-top:12px;font:600 15px Arial;line-height:1.45;}
...
try:
    ...
    with control_1:
        audience = st.selectbox(
            "Audience",
            audiences,
            index=audiences.index("All Audiences"),
            format_func=lambda value: "Total BBC Audience" if value == "All Audiences" else value,
        )
    with control_2:
    ...
    unavailable_regions = [name for name in repo.paths if name not in regions]
    if unavailable_regions:
        st.caption(
            "Regional workbooks still using the previous row structure are temporarily hidden: "
            + ", ".join(unavailable_regions)
            + ". They will appear automatically when their new exports are uploaded."
        )
    ...
    st.markdown(
        ...
        <div class="ai-card">
          ...
          <div class="interpretation-note"><strong>How to read this:</strong> {overview.interpretation_note}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        '<div class="grounded-note">Helps users understand the evidence in the audience slides, '
        "compare supported results and find where information appears. It explains what the data "
        "shows without making claims beyond the available evidence. No external AI service is used.</div>",
        unsafe_allow_html=True,
    )
    with st.form("audience_ai_form", clear_on_submit=True):
        ...
        with ask_col:
            question = st.text_input(
                "Question",
                placeholder='Ask about a slide, audience, metric or GWI methodology',
    ...
    for previous_question, answer in reversed(st.session_state.audience_ai_history):
        with st.container(border=True):
            ...
            if answer.relevant_slide:
                st.button(
                    f"View relevant slide {answer.relevant_slide}",
                    key=f"view_slide_{len(st.session_state.audience_ai_history)}_{previous_question}_{answer.relevant_slide}",
                    on_click=request_template_slide,
                    args=(answer.relevant_slide,),
                )
            if answer.evidence:
            ...
            feedback_key = f"feedback_{previous_question}_{answer.resolved_scope}"
            if feedback_key not in st.session_state:
                feedback_left, feedback_right, _ = st.columns([1.2, 1.2, 7.6])
                with feedback_left:
                    if st.button("Helpful", key=f"helpful_{feedback_key}"):
                        st.session_state[feedback_key] = "helpful"
                with feedback_right:
                    if st.button("Not helpful", key=f"unhelpful_{feedback_key}"):
                        st.session_state[feedback_key] = "not helpful"
            else:
                st.caption("Feedback recorded for this session. No question text or workbook data is sent externally.")
    ...
    if pages:
        ...
        requested_template_slide = st.session_state.pop("requested_template_slide", None)
        if requested_template_slide is not None:
            matching_positions = [
                position for position, slide in enumerate(model.slides, 1)
                if slide.number == requested_template_slide
            ]
            if matching_positions:
                st.session_state.slide_number = matching_positions[0]
            else:
                st.info(
                    f"Slide {requested_template_slide} is not included in this specialist deck, "
                    "although the chatbot can still use its validated source data."
                )
    ...
    # Keep export controls directly beneath the slide preview so they remain
    # easy to find after the user has reviewed the presentation.
    st.markdown("### Download presentation")
    base_name = f"BBC_Audience_Deck_{audience}_{region}_{platform}".replace(" ", "_")
    with st.popover("Download presentation ▾"):
