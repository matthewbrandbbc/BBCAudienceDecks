...
try:
    ...
    st.markdown(
        '<div class="ask-label">Ask the Audience AI</div>'
    ...
    for suggestion_col, suggestion in zip(
        suggestion_cols, engine.suggested_questions(region, audience, platform)
    ):
        with suggestion_col:
            if st.button(suggestion, use_container_width=True):
                suggested_question = suggestion

    submitted_question = question.strip() if ask_submitted else suggested_question
